...
# ═══════════════════ 可编辑配置（WebUI 插件管理可直接识别）═══════════════════
# 修改后重启 Bot 生效；也可通过 .env (HELP_XXX) 覆盖
COMMAND = '帮助'                               # 帮助指令触发词
TITLE = '流萤Help'                              # 帮助图大标题
SUBTITLE = '输入 /帮助 关键词 可筛选具体功能'        # 帮助图副标题
DISPLAY_MODE = 'auto'                           # 显示模式: auto=自动分类, flat=平坦单插件
EXCLUDE_NAMES = '__pycache__'          # 从帮助图中排除的插件名列表
