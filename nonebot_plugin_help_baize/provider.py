...
from .config import CONFIG, PLUGINS_ROOT, resolve_category
...
def _match_triggers_to_meta(
    ...
    for trigger in triggers:
        ...
        # ── Priority 1: Start-of-line match in usage_lines ──
        for line in usage_lines:
            if line.startswith('/' + trigger) or line.startswith(trigger):
                idx = line.find(trigger)
                rest = line[idx + len(trigger):]
                m = re.search(r'\s+(.+)', rest)
                if m:
                    desc = m.group(1)
                break

        # ── Priority 2: Substring match in usage_lines (fallback) ──
        if not desc:
            for line in usage_lines:
                if trigger in line:
                    idx = line.find(trigger)
        ...
        # ── Priority 3: Search in full metadata blob ──
        if not desc:
            for line in meta_blob_lines:
                if trigger in line:
                    idx = line.find(trigger)
                    rest = line[idx + len(trigger):]
                    m = re.search(r'\s+(.+)', rest)
...
def _split_usage_sections(usage_lines: List[str]) -> Tuple[List[str], List[str], List[str]]:
    ...
    for raw_line in usage_lines:
        ...
        if " " in line:
            ...
            desc = parts[1].strip()
            # 触发词以 / 或中文/英文开头，且不为常见描述性短语开头
            if first_word.startswith("/") or (
                len(first_word) <= 12 and not any(
                    kw in first_word for kw in ("说明", "用法", "示例", "比如", "例如", "支持", "可以", "需要", "提示")
                )
            ):
...
def _iter_plugin_sources() -> Iterable[Tuple[str, Path]]:
    """Yield (plugin_id, source_path) for local plugins under PLUGINS_ROOT."""
...
def _iter_external_plugin_sources() -> Iterable[Tuple[str, Path]]:
    ...
    (pip-installed) that are NOT already local plugins under PLUGINS_ROOT.
    ...
    for plugin in plugins:
        ...
        # Skip if the file lives inside PLUGINS_ROOT (already covered)
        try:
...
def _entry_from_path(plugin_id: str, path: Path) -> Optional[HelpEntry]:
    ...
    if matcher_triggers:
        ...
        # ── Phase 3b: Fallback — no matchers found, extract from usage text ──
        triggers, trigger_descs, _ = _split_usage_sections(usage_lines)
        if not triggers:
            triggers = _extract_commands_from_usage(usage_lines)
        if not triggers:
            triggers = [cmd.lstrip("/") for cmd in usage_lines if cmd.strip().startswith("/")]
        if not triggers:
            triggers = _extract_commands(text)
        # Pad descriptions to match triggers length
        while len(trigger_descs) < len(triggers):
            trigger_descs.append('')
...
def load_help_entries() -> List[HelpEntry]:
    ...
    # 排序：优先用插件级 sort_order（>0 时），否则用模块级 sort_order
    ...
    def _sort_key(item: HelpEntry):
        ...
        # 主键=模块排序（自动分类模式），次键=插件排序（平坦模式/模块内排序），末键=名称
